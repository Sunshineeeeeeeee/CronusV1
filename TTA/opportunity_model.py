...
class BayesianLinear(nn.Module):
    """Bayesian Linear Layer with Gaussian weight uncertainty. GPU-optimized."""
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True, 
        ...
        # Weight priors - store as buffers for GPU operation
        self.register_buffer('prior_sigma_1', torch.tensor(prior_sigma_1))
        self.register_buffer('prior_sigma_2', torch.tensor(prior_sigma_2))
        self.register_buffer('prior_pi', torch.tensor(prior_pi))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Get device from input for consistent placement
        device = x.device
    ...
    def sample_weights(self) -> torch.Tensor:
        """Sample weights using the reparameterization trick. GPU-optimized."""
        # Fuse log1p and exp operations for better GPU utilization
        weight_sigma = F.softplus(self.weight_rho)  # More stable than log1p(exp())
        
        if self.training:
            # Generate noise directly on the GPU with proper shape
            weight_epsilon = torch.randn_like(weight_sigma, device=weight_sigma.device)
            return self.weight_mu + weight_epsilon * weight_sigma
        else:
            return self.weight_mu
    
    def sample_bias(self) -> torch.Tensor:
        """Sample bias using the reparameterization trick. GPU-optimized."""
        if self.bias:
            # Fuse log1p and exp operations for better GPU utilization
            bias_sigma = F.softplus(self.bias_rho)  # More stable than log1p(exp())
            
            # Generate noise directly on the GPU with proper shape
            bias_epsilon = torch.randn_like(bias_sigma, device=bias_sigma.device)
            return self.bias_mu + bias_epsilon * bias_sigma
        else:
            return None
    
    def kl_loss(self) -> torch.Tensor:
        """Calculate the KL divergence between the posterior and prior of weights. GPU-optimized."""
        ...
        kl_loss = self._kl_divergence(
            ...
            self.prior_sigma_1.pow(2), self.prior_sigma_2.pow(2), self.prior_pi
        )
    ...
    def _kl_divergence(self, mu_q, sigma_q, mu_p, sigma_p1, sigma_p2, pi):
        """Calculate the KL divergence between a Gaussian mixture posterior and prior. GPU-optimized."""
        ...
        # Pre-compute common terms for better parallelism
        sqrt_sigma_q = torch.sqrt(sigma_q)
        sqrt_sigma_p1 = torch.sqrt(sigma_p1)
        sqrt_sigma_p2 = torch.sqrt(sigma_p2)
        
        # Vectorized computation for term1
        term1 = torch.log(pi * sqrt_sigma_p1 / sqrt_sigma_q + 
                         (1 - pi) * sqrt_sigma_p2 / sqrt_sigma_q)
        
        # Compute squared difference once
        mu_diff_squared = (mu_q - mu_p).pow(2)
        
        # Vectorized computation for term2
        term2 = -0.5 + 0.5 * (pi * (sigma_q + mu_diff_squared) / sigma_p1 + 
                            (1 - pi) * (sigma_q + mu_diff_squared) / sigma_p2)
...
class DirectionPredictor:
    ...
    GPU-optimized with efficient tensor operations.
    """
    
    def __init__(self, threshold=0.2, adaptive=True, up_threshold=None, down_threshold=None, device=None):
        """
        ...
        # Store thresholds as tensors on the appropriate device
        self.base_threshold = torch.tensor(threshold, device=self.device)
        self.adaptive = adaptive
        
        # Allow for asymmetric thresholds to handle class imbalance
        self.up_threshold = torch.tensor(up_threshold if up_threshold is not None else threshold, device=self.device)
        self.down_threshold = torch.tensor(down_threshold if down_threshold is not None else threshold, device=self.device)
    ...
    def to(self, device):
        ...
        """
        self.device = device
        self.base_threshold = self.base_threshold.to(device)
        self.up_threshold = self.up_threshold.to(device)
        self.down_threshold = self.down_threshold.to(device)
        return self
    
    def update_thresholds(self, true_directions, pred_means):
        """
        Update thresholds based on observed data distribution.
        GPU-optimized with vectorized operations.
        ...
        device = true_directions.device
        # Move thresholds to the same device as inputs if needed
        if device != self.device:
            self.to(device)
            
        # Count directions using efficient tensor operations
        up_mask = (true_directions > 0)
        down_mask = (true_directions < 0)
        neutral_mask = ~(up_mask | down_mask)
        
        up_count = up_mask.sum().item()
        down_count = down_mask.sum().item()
        neutral_count = neutral_mask.sum().item()
        total = len(true_directions)
        
        # Calculate class weights (inverse frequency) with safe division
        total_float = float(total)
        weights = {}
        weights['up'] = total_float / max(up_count, 1)
        weights['down'] = total_float / max(down_count, 1)
        weights['neutral'] = total_float / max(neutral_count, 1)
        
        # Normalize weights with a single operation
        ...
        base = self.base_threshold.item()  # Get scalar value for calculations
        
        # Compute thresholds using vectorized operations where possible
        if weights['up'] > weights['down']:
            # Up class is underrepresented, make up predictions easier
            up_thresh = base * (1.0 - 0.5 * (weights['up'] - weights['down']))
            down_thresh = base * (1.0 + 0.3 * (weights['up'] - weights['down']))
        elif weights['down'] > weights['up']:
            # Down class is underrepresented, make down predictions easier
            up_thresh = base * (1.0 + 0.3 * (weights['down'] - weights['up']))
            down_thresh = base * (1.0 - 0.5 * (weights['down'] - weights['up']))
        else:
            # Balanced
            up_thresh = down_thresh = base
            
        # Ensure thresholds are reasonable with vectorized clamp
        self.up_threshold = torch.tensor(max(0.05, min(0.5, up_thresh)), device=self.device)
        self.down_threshold = torch.tensor(max(0.05, min(0.5, down_thresh)), device=self.device)
    ...
    def predict_direction(self, trend_mean, trend_var=None, calibrate=False, true_directions=None):
        ...
        # Pre-compute condition masks for better parallelism
        up_condition = trend_mean > self.up_threshold
        down_condition = trend_mean < -self.down_threshold
        
        # Apply asymmetric thresholds to determine direction in a single vectorized operation
        direction = torch.zeros_like(trend_mean)
        direction = torch.where(up_condition, torch.ones_like(trend_mean), direction)
        direction = torch.where(down_condition, -torch.ones_like(trend_mean), direction)
        
        # Calculate confidence based on signal strength and variance
        if trend_var is not None:
            # Pre-compute masks for better parallelism
            up_mask = (trend_mean > 0)
            down_mask = (trend_mean < 0)
            neutral_mask = ~(up_mask | down_mask)
            
            # Initialize confidence tensor
            confidence = torch.zeros_like(trend_mean, device=device)
            
            # Process each direction type with optimized operations
            # Up direction confidence
            if up_mask.any():
                # Use where operation for better parallelism
                up_confidence = torch.abs(trend_mean[up_mask]) / (self.up_threshold * (1.0 + torch.sqrt(trend_var[up_mask])))
                confidence[up_mask] = up_confidence
            
            # Down direction confidence
            if down_mask.any():
                down_confidence = torch.abs(trend_mean[down_mask]) / (self.down_threshold * (1.0 + torch.sqrt(trend_var[down_mask])))
                confidence[down_mask] = down_confidence
            
            # Neutral confidence
            if neutral_mask.any():
                # Vectorized minimum operation for improved performance
                up_thresh_distance = torch.abs(self.up_threshold - trend_mean[neutral_mask])
                down_thresh_distance = torch.abs(-self.down_threshold - trend_mean[neutral_mask])
                threshold_distance = torch.minimum(up_thresh_distance, down_thresh_distance)
                
                max_thresh = torch.max(self.up_threshold, self.down_threshold)
                neutral_confidence = 1.0 - threshold_distance / max_thresh
                confidence[neutral_mask] = neutral_confidence
        else:
            # Simpler vectorized confidence calculation without variance
            confidence = torch.zeros_like(trend_mean, device=device)
...
class FocalLoss(nn.Module):
    ...
    def __init__(self, gamma=3.0, alpha=0.25, auto_weight=True, device=None):
        """
        ...
        # Get default device if not provided
        self.device = device if device is not None else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
        # Register parameters as tensors for consistent device handling
        self.register_buffer('gamma', torch.tensor(gamma, device=self.device))
        self.register_buffer('alpha', torch.tensor(alpha, device=self.device))
        
        self.auto_weight = auto_weight
        self.class_weights = {
            'up': torch.tensor(1.0, device=self.device),
            'neutral': torch.tensor(1.0, device=self.device),
            'down': torch.tensor(1.0, device=self.device)
        }
    ...
    def to(self, device):
        ...
        for k in self.class_weights:
            self.class_weights[k] = self.class_weights[k].to(device)
    ...
    def update_weights(self, true_direction):
        ...
        device = true_direction.device
        
        # Move weights to the same device as input if needed
        if device != self.device:
            self.to(device)
            
        # Compute class masks for efficient counting
        up_mask = (true_direction > 0)
        down_mask = (true_direction < 0)
        neutral_mask = ~(up_mask | down_mask)
        
        # Count classes using tensor operations
        up_count = up_mask.sum().item()
        down_count = down_mask.sum().item()
        neutral_count = neutral_mask.sum().item()
        ...
        # Avoid division by zero with safe values
        up_count = max(up_count, 1)
        down_count = max(down_count, 1)
        neutral_count = max(neutral_count, 1)
            
        # Calculate inverse frequency weights
        total_float = float(total)
        up_weight = total_float / up_count
        down_weight = total_float / down_count
        neutral_weight = total_float / neutral_count
        
        # Normalize weights to sum to class count (3)
        weight_sum = up_weight + down_weight + neutral_weight
        self.class_weights['up'] = torch.tensor(3.0 * up_weight / weight_sum, device=device)
        self.class_weights['down'] = torch.tensor(3.0 * down_weight / weight_sum, device=device)
        self.class_weights['neutral'] = torch.tensor(3.0 * neutral_weight / weight_sum, device=device)
    
    def forward(self, pred_means, true_direction, sample_weights=None):
        """
        Calculate focal loss for direction prediction.
        GPU-optimized with fused operations.
        ...
        Returns:
            Focal loss
        """
        device = pred_means.device
        
        # Update weights if using auto-weighting
        if self.auto_weight:
            self.update_weights(true_direction)
            
        # Ensure inputs have correct dimensions with vectorized reshape
        ...
        # Convert direction to class indices (0, 1, 2) using efficient mapping
        # Map [-1, 0, 1] to [0, 1, 2]
        target = true_direction.long() + 1  
        
        # Convert predictions to directional probabilities
        # Using sigmoid to map trend strength to [0, 1]
        # Compute all probabilities in one pass for better parallelism
        pos_probs = torch.sigmoid(pred_means)  # Probability of uptrend
        neg_probs = torch.sigmoid(-pred_means)  # Probability of downtrend
        
        # Ensure neutral probabilities are valid with safe ops
        neutral_probs = 1.0 - pos_probs - neg_probs 
        ...
        # One-hot encode targets efficiently
        batch_size = probs.size(0)
        target_one_hot = torch.zeros(batch_size, 3, device=device)
        target_one_hot.scatter_(1, target.unsqueeze(1), 1)
        
        # Apply class weights using vectorized operations
        ...
        # Compute once and reuse for better efficiency
        probs_powered = probs.pow(self.gamma)
        inv_probs_powered = (1 - probs).pow(self.gamma)
        
        focal_weights = inv_probs_powered * target_one_hot + probs_powered * (1 - target_one_hot)
        ...
        # Use efficient log with small epsilon for stability
        log_probs = torch.log(probs + self.eps)
        log_inv_probs = torch.log(1 - probs + self.eps)
        
        loss = -weights * (target_one_hot * log_probs + (1 - target_one_hot) * log_inv_probs)
...
class MultiHeadAttention(nn.Module):
    ...
    GPU-optimized with fused operations and better memory management.
    """
    def __init__(self, dim, num_heads=4, dropout=0.1):
        ...
        self.scale = 1.0 / math.sqrt(self.head_dim)  # Pre-compute scale for better efficiency
        
        # Use a single projection for better parallelism, then split
        ...
        # Register attention pattern for monitoring (optional)
        self.register_buffer("attn_pattern", None)
        
    def forward(self, x, return_attention=False):
        """GPU-optimized forward pass with fused operations."""
        # Input shape: [batch_size, seq_len, dim]
        batch_size, seq_len, _ = x.shape
        device = x.device
        
        # Store residual before normalization for later addition
        residual = x
        
        # Apply normalization before attention (layer norm is already in-place)
        x = self.norm(x)
        
        # Project queries, keys, values with a single projection for better parallelism
        # Shape: [batch_size, seq_len, 3*dim]
        qkv = self.qkv_proj(x)
        ...
        # Split along the third dimension and permute for correct shape
        # Shape: 3 x [batch_size, num_heads, seq_len, head_dim]
        q, k, v = qkv.unbind(dim=2)
        q = q.permute(0, 2, 1, 3)  # [batch_size, num_heads, seq_len, head_dim]
        k = k.permute(0, 2, 1, 3)
        v = v.permute(0, 2, 1, 3)
        
        # Compute attention scores with pre-computed scale
        # Use more stable operation sequence
        ...
        # Store attention pattern if requested
        if return_attention:
            self.attn_pattern = attn_weights.detach()
        
        # Apply attention to values
        # Shape: [batch_size, num_heads, seq_len, head_dim]
        context = torch.matmul(attn_weights, v)
        
        # Reshape context back to original shape
        ...
        # Apply residual connection (in-place add to improve memory usage)
        output.add_(residual)
        
        if return_attention:
            return output, self.attn_pattern
        return output
...
class TradingOpportunityModel(nn.Module):
    ...
    def __init__(
        ...
        # Enable AMP (Automatic Mixed Precision) if requested
        self.use_amp = use_amp
        if self.use_amp and self.device.type != 'cuda':
            print("Warning: AMP requires CUDA. Disabling AMP.")
            self.use_amp = False
            
        # Create scaler for AMP if enabled
        ...
        # Direction predictor
        self.direction_predictor = DirectionPredictor(threshold=0.1, device=self.device)
    ...
    def forward(self, features, regimes=None, subregimes=None, time_features=None, return_attention=False):
        ...
        Uses AMP if enabled for faster computation on compatible hardware.
        ...
        """
        # Use automatic mixed precision for better performance if enabled
        if self.use_amp and self.training:
            with torch.cuda.amp.autocast():
                return self._forward_impl(features, regimes, subregimes, time_features, return_attention)
        else:
            return self._forward_impl(features, regimes, subregimes, time_features, return_attention)
    
    def _forward_impl(self, features, regimes=None, subregimes=None, time_features=None, return_attention=False):
        """
    ...
    def nt_xent_loss(self, embeddings, labels=None, temperature=0.5, batch_size=None):
        """
        Compute the NT-Xent (Normalized Temperature-scaled Cross Entropy) contrastive loss.
        GPU-optimized with fused operations and memory-efficient tensor handling.
        ...
        Args:
            ...
            batch_size: Optional parameter to process in smaller batches to save memory
        ...
        # Use provided batch size or process all at once
        process_batch_size = batch_size or full_batch_size
        
        # For very large batches, process in sub-batches to avoid OOM
        if full_batch_size > process_batch_size and batch_size is not None:
            # Process in chunks to save memory
            total_loss = 0.0
            num_chunks = math.ceil(full_batch_size / process_batch_size)
            
            for i in range(num_chunks):
                start_idx = i * process_batch_size
                end_idx = min((i + 1) * process_batch_size, full_batch_size)
                
                # Process current chunk
                chunk_embeddings = embeddings[start_idx:end_idx]
                chunk_labels = labels[start_idx:end_idx] if labels is not None else None
                
                # Calculate loss for this chunk (without the batch_size parameter to avoid recursion)
                chunk_loss = self.nt_xent_loss(chunk_embeddings, chunk_labels, temperature, None)
                total_loss += chunk_loss * (end_idx - start_idx)
            
            # Average loss across all chunks
            return total_loss / full_batch_size
        
        # Normalize embeddings for cosine similarity in a single fused operation
        # This avoids creating an additional tensor
        embeddings_norm = F.normalize(embeddings, p=2, dim=1)
        
        # Compute similarity matrix efficiently 
        # This is one of the most GPU-intensive operations
        similarity_matrix = torch.matmul(embeddings_norm, embeddings_norm.transpose(0, 1)) / temperature
        
        # Create mask for self-similarity on the same device as the embeddings
        mask = torch.eye(full_batch_size, dtype=torch.bool, device=device)
        
        # Convert similarities to a masked version
        # Apply mask in-place to avoid additional memory allocation
        # Fill diagonal with large negative value to exclude self-pairs
        similarity_matrix.masked_fill_(mask, -10.0)
        
        # If labels are provided, use them to define positive pairs (supervised)
        if labels is not None and not torch.all(labels == 0):
            # Compute label equality matrix efficiently
            # Create expanded labels matrix on GPU directly
            labels_expanded = labels.unsqueeze(1).expand(full_batch_size, full_batch_size)
            labels_expand_t = labels.unsqueeze(0).expand(full_batch_size, full_batch_size)
            labels_equal = (labels_expanded == labels_expand_t)
            
            # Remove self-pairs
            labels_equal.masked_fill_(mask, False)
            
            # Compute positive and negative similarities in one operation
            pos_mask = labels_equal
            neg_mask = ~labels_equal & ~mask
            
            # Compute exponentials once
            exp_sim = torch.exp(similarity_matrix)
            
            # Compute positive and negative sums
            pos_similarities = exp_sim * pos_mask.float()
            neg_similarities = exp_sim * neg_mask.float()
            ...
            # Find samples with positive pairs
            ...
            if valid_mask.sum() > 0:
                ...
                # Compute loss directly without creating intermediate tensors
                log_ratio = torch.log(valid_pos_sum / valid_neg_sum)
                per_sample_loss = -log_ratio.squeeze()
                
                # Average only over samples with valid pairs
                loss = (per_sample_loss * valid_mask).sum() / (valid_mask.sum() + 1e-6)
            else:
    ...
    def _unsupervised_nt_xent(self, similarity_matrix):
        ...
        """
        # Use stable implementation with logsumexp
        # Instead of computing exp and then log, use logsumexp directly
        # This is numerically more stable and more efficient on GPU
        
        # Pre-compute exponentials once (reusing the existing matrix)
        exp_sim = torch.exp(similarity_matrix)
        
        # Sum over all samples to get denominator using a single reduction
        # This is more GPU-efficient than doing multiple operations
        exp_sim_sum = torch.sum(exp_sim, dim=1)
        
        # Use log1p when needed for better numerical stability
        # Compute loss directly: -log(exp_sim_sum)
        # Apply clamp in-place to avoid additional tensor allocation
        loss = -torch.log(torch.clamp(exp_sim_sum, min=1e-6))
        
        # Return mean with stable reduction
        return loss.mean()
    ...
    def generate_adversarial_examples(self, features, regimes=None, subregimes=None, time_features=None, 
                                      trend_true=None, epsilon=0.01, batch_size=None):
        """
        Generate adversarial examples using Fast Gradient Sign Method (FGSM).
        GPU-optimized with efficient tensor operations and optional batch processing.
        ...
        Args:
            ...
            batch_size: Optional batch size for processing large inputs in chunks
            
        Returns:
            Adversarial examples
        """
        # Process in batches if specified and needed
        if batch_size is not None and features.size(0) > batch_size:
            ...
            for i in range(total_batches):
                ...
                batch_regimes = regimes[start_idx:end_idx] if regimes is not None else None
                batch_subregimes = subregimes[start_idx:end_idx] if subregimes is not None else None
                batch_time_features = time_features[start_idx:end_idx] if time_features is not None else None
                batch_trend_true = trend_true[start_idx:end_idx] if trend_true is not None else None
                
                # Process batch (without batch_size to avoid recursion)
                batch_adv = self.generate_adversarial_examples(batch_features, batch_regimes, batch_subregimes,
                                                             batch_time_features, batch_trend_true, epsilon, None)
                adv_batches.append(batch_adv)
        ...
        # Use memory-efficient implementation to compute loss
        ...
        # Generate adversarial examples using FGSM with in-place operations when possible
        grad_sign = features_adv.grad.sign()
        
        # Use vectorized operations for creating adversarial examples
        features_min = features.min()
        features_max = features.max()
        
        # In-place perturbation
        perturbation = epsilon * grad_sign
        features_adv = features.detach() + perturbation
        
        # In-place clamping to valid range
        features_adv.clamp_(features_min, features_max)
        
        # Clean up to avoid memory leaks
        del grad_sign, perturbation
