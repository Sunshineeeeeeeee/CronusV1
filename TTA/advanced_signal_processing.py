...
class SignalProcessor:
    ...
    def wavelet_denoising(self, data: np.ndarray, level: int = None) -> np.ndarray:
        ...
        if padding > 0:
            data_padded = np.pad(data, (0, padding), mode='symmetric')
        else:
            data_padded = data
            
        # Calculate appropriate level if not provided
        if level is None:
            max_level = pywt.dwt_max_level(len(data_padded), self.wavelet)
            level = min(3, max_level)
            
        # Decompose
        coeffs = pywt.wavedec(data_padded, self.wavelet, level=level)
        
        # Threshold details coefficients
        for i in range(1, len(coeffs)):
            # Handle division by zero warning
            # Use our own thresholding to avoid PyWavelets division warning
            details = coeffs[i]
            thresh_val = self.denoising_threshold * np.max(np.abs(details)) if np.max(np.abs(details)) > 0 else 0
            
            if thresh_val > 0:
                # Apply soft thresholding manually to avoid warning
                magnitude = np.abs(details)
                # Create a mask to avoid division by zero
                mask = magnitude > 0
                result = np.zeros_like(details)
                # Only apply thresholding where magnitude is non-zero
                if np.any(mask):
                    # Soft thresholding: sign(x) * max(0, |x| - threshold)
                    with np.errstate(divide='ignore', invalid='ignore'):
                        thresholded = (1 - thresh_val/magnitude)
                    thresholded[~mask] = 0  # Set to zero where magnitude is zero
                    thresholded[thresholded < 0] = 0  # Clamp negative values
                    result = details * thresholded
                coeffs[i] = result
    ...
    def path_signatures(self, data: np.ndarray, depth: int = 2) -> Dict[str, np.ndarray]:
        ...
        try:
            ...
            try:
                ...
                # Use first order terms for trend estimation
                # Extract the relevant terms from the signature
                ...
                #print("Using iisignature package instead of signatory.")
                
            except ImportError:
                # If signatory and iisignature are not available, use an enhanced approximation
                #print("Warning: 'signatory' and 'iisignature' packages not found. Using enhanced signature approximation.")
                #print("Install with: pip install iisignature")
    ...
    def process_price_series(self, prices: np.ndarray, include_methods: List[str] = None) -> Dict[str, np.ndarray]:
        ...
        if 'kalman' in methods_to_use:
            # Kalman filtering - suitable for HFT as it handles noise well
        ...
        if 'changepoint' in methods_to_use:
            ...
            changepoint_trend = changepoint_result.get('trend', changes)
            method_results['changepoint'] = {'trend': changepoint_trend}
            
        if 'hht' in methods_to_use:
            # Hilbert-Huang Transform - can struggle with HFT noise, handle with care
            try:
